...
Decision Decision_RealizeGoal(Event *goal, long currentTime)
{
    ...
    Concept *best_prec = NULL;
    Concept *best_postc = NULL;
    for(int postc_i=0; postc_i<concepts.itemsAmount; postc_i++)
    {
        Concept *postc = concepts.items[postc_i].address;
        for(int opi=1; opi<OPERATIONS_MAX; opi++)
        {
            ...
            for(int j=0; j<postc->precondition_beliefs[opi].itemsAmount; j++)
            {
                ...
                if(SDR_Equal(&current_prec->sdr, &imp.sourceConceptSDR))
                {
                    ...
                    {
                        Event goal_ = Memory_MatchEventToConcept(postc, goal); 
                        Event ContextualOperation = Inference_GoalDeduction(&goal_, &imp); //(&/,a,op())! :\:
    ...
    {
        if(best_postc->anticipation_deadline[i] == 0)
        {
            ...
            best_postc->anticipation_deadline[i] = currentTime + MAX(ANTICIPATION_MIN_WINDOW, ANTICIPATION_FORWARD_MUL * bestImp.occurrenceTimeOffset + pessimistic_stddev_estimate * ANTICIPATION_VARIANCE_MUL);
            best_postc->anticipation_negative_confirmation[i] = bestImp;
            best_postc->anticipation_negative_confirmation[i].truth = (Truth) { .frequency = 0.0, .confidence = c };
            best_postc->anticipation_negative_confirmation[i].stamp = (Stamp) { .evidentalBase = { -stampID } };
            best_postc->anticipation_operation_id[i] = decision.operationID;
